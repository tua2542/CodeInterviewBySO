...
function circularArrayRotation(a, k, queries) {
    let n = a.length;
    let effectiveR = k % n;
    let rotatedArr = a.slice(n - effectiveR).concat(a.slice(0, n - effectiveR));
    let result = queries.map(queryIndex => rotatedArr[queryIndex]);
    return result;
...
function circularArrayRotation(a, k, queries) {
    ...
    const result = queries.map(queryIndex => rotatedArray[queryIndex]);
